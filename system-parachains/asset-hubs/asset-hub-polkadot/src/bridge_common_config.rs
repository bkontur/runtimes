...
pub enum BridgeReward {
	/// Rewards for the K/P bridge—distinguished by the `RewardsAccountParams` key.
	KusamaPolkadot(RewardsAccountParams<HashedLaneId>),
}

impl From<RewardsAccountParams<HashedLaneId>> for BridgeReward {
	fn from(value: RewardsAccountParams<HashedLaneId>) -> Self {
		Self::KusamaPolkadot(value)
	}
}
...
impl bp_relayers::PaymentProcedure<AccountId, BridgeReward, u128> for BridgeRewardPayer {
	...
	fn pay_reward(
		...
		match reward_kind {
			BridgeReward::KusamaPolkadot(lane_params) => {
				match beneficiary {
					...
					BridgeRewardBeneficiaries::AssetHubLocation(_) => Err(Self::Error::Other("`AssetHubLocation` beneficiary is not supported for `KusamaPolkadot` rewards!")),
				}
			}
		}
	}
